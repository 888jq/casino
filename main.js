...
let balanceDisplay, winDisplay, freeSpinsDisplay, betField, spinBtn, cascadeMultiplierDisplay;
...
function initDisplay() {
  ...
  cascadeMultiplierDisplay = document.getElementById('cascadeMultiplier');
  ...
  console.log("Display elements initialized:", {
    ...
    betField: !!betField,
    spinBtn: !!spinBtn,
    cascadeMultiplierDisplay: !!cascadeMultiplierDisplay
  });
}
...
// Update cascade multiplier display
function updateCascadeMultiplier(cascadeCount, isFreeSpin = false) {
  if (!cascadeMultiplierDisplay) cascadeMultiplierDisplay = document.getElementById('cascadeMultiplier');
  if (!cascadeMultiplierDisplay) return;
  
  let multiplier;
  if (!isFreeSpin) {
    if (cascadeCount === 0) multiplier = '×1';
    else if (cascadeCount === 1) multiplier = '×2';
    else if (cascadeCount === 2) multiplier = '×3';
    else multiplier = '×5';
  } else {
    if (cascadeCount === 0) multiplier = '×2';
    else if (cascadeCount === 1) multiplier = '×4';
    else if (cascadeCount === 2) multiplier = '×6';
    else multiplier = '×10';
  }
  
  cascadeMultiplierDisplay.textContent = multiplier;
  
  // Trigger pop animation
  cascadeMultiplierDisplay.classList.remove('cascade-pop');
  void cascadeMultiplierDisplay.offsetWidth; // Force reflow to restart animation
  cascadeMultiplierDisplay.classList.add('cascade-pop');
  
  console.log("Cascade multiplier updated to: " + multiplier);
}
...
// Trigger WIN display animation (flash and glow)
function triggerWinAnimation() {
  if (!winDisplay) winDisplay = document.getElementById('winDisplay');
  if (!winDisplay) return;
  
  // Find the parent stat-box
  const winBox = winDisplay.closest('.stat-box');
  
  // Remove animation classes if they exist
  winDisplay.classList.remove('win-flash');
  if (winBox) winBox.classList.remove('win-glow');
  
  // Force reflow to restart animation
  void winDisplay.offsetWidth;
  void winBox?.offsetWidth;
  
  // Add animation classes
  winDisplay.classList.add('win-flash');
...
initDisplay();
updateDisplay();
updateCascadeMultiplier(0, false); // Show ×1 initially
...
function roll(isFreeSpin = false){
  ...
  setTimeout(() => {
    ...
    while (cascadeCount < maxCascades){
      ...
      // Update cascade multiplier display
      updateCascadeMultiplier(cascadeCount, isFreeSpin);
    ...
    // Reset cascade multiplier display to ×1
    updateCascadeMultiplier(0, false);
...
function getRandomNum(){
  for (let i = 0; i < slotsRan.length; i++){
    for (let j = 0; j < slotsRan[0].length; j++){
      random = Math.floor(Math.random() * 522) + 1;
      slotsRan[i][j] = random;
